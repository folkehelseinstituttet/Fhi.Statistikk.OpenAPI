...
export class OpenApiService {
  ...
  getDimensions(sourceId: string, tableid: number): Observable<DimensionsArray> {
    const endpoint = sourceId + '/Table/' + tableid + '/dimension';
    const url = this.apiUrl + endpoint;
    return this.http.get<DimensionsArray>(url, {
      withCredentials: true,
    }).pipe(
      catchError((error: HttpErrorResponse) => {
        if (error.status === 500) {
  ...
  getQuery(sourceId: string, tableId: number): Observable<DataQuery> {
    ...
    return this.http.get<DataQuery>(url, {
      withCredentials: true,
    }).pipe(
      catchError((error: HttpErrorResponse) => {
        if (error.status === 500) {
  ...
  getData(
    ...
    const url = this.apiUrl + endpoint;
    return this.http.post<Dataset>(url, request, {
      withCredentials: true,
    }).pipe(
      catchError((error: HttpErrorResponse) => {
        if (error.status === 500) {+
